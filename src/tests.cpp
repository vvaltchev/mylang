...
static const std::vector<test> tests =
{
    ...
    /* ---- exit / type builtins (types.cpp) ---- */
    {
        "kindstr() value forms",
        {
            ...
            "assert(kindstr(2.5) == \"float\");",
            "assert(kindstr(\"s\") == \"str\");",
            "assert(kindstr([1]) == \"array\");",
    ...
    { "ternary: right-associative chain",
      { "var x = 2;",
        ...
        "assert((x == 9 ? \"a\" : x == 8 ? \"b\" : \"c\") == \"c\");" } },
    { "ternary: condition is any truthy value (like if)",
      { "assert((5 ? 1 : 2) == 1); assert((0 ? 1 : 2) == 2);",
        "assert((\"x\" ? 1 : 2) == 1); assert(([] ? 1 : 2) == 2);" } },
    { "ternary: branch result type is the join (int/float -> float)",
      { "var dyn x = runtime(1); var r = x > 0 ? 1 : 2.0;",
        "assert(r == 1.0);" } },
    { "ternary: only the taken branch is evaluated",
      { "var hit = 0;",
        "func bump() { hit = hit + 1; return 9; }",
        "var r = runtime(true) ? 1 : bump(); assert(r == 1);",
        "var r2 = runtime(false) ? bump() : 2; assert(r2 == 2);",
        "assert(hit == 0);" } },
    { "coalesce: none -> rhs, value -> lhs",
      { "var dyn a = none; assert((a ?? 5) == 5);",
        "var dyn b = 7; assert((b ?? 5) == 7);",
        "assert((none ?? 3) == 3);" } },
    { "coalesce: right-associative",
      { "var dyn a = none; var dyn b = none;",
        "assert((a ?? b ?? 9) == 9);" } },
    { "coalesce: rhs not evaluated when lhs is non-none",
      { "var hit = 0;",
        "func bump() { hit = hit + 1; return 0; }",
        "var dyn v = 7; var dyn r = v ?? bump();",
        "assert(r == 7); assert(hit == 0);" } },
    { "coalesce: an opt value defaults to a non-opt (usable without a check)",
      { "int? a; var b = a ?? 0; assert(b == 0); assert(b + 1 == 1);" } },
    { "precedence: || binds tighter than ?:",
      { "assert((true || false ? 1 : 2) == 1);",
        "assert((false || false ? 1 : 2) == 2);" } },
    { "precedence: ?? binds tighter than ?:",
      { "var dyn z = none;",
        "assert((z ?? 1 > 0 ? \"Y\" : \"N\") == \"Y\");" } },
    { "ternary / coalesce const-fold at parse time",
      { "const C = true ? 11 : 22; assert(C == 11);",
        "const D = none ?? 33; assert(D == 33);",
        "const E = 5 ?? 99; assert(E == 5);" } },
    { "ternary const-fold: an auto-const flag guard collapses",
      { "var flag = false;",     /* write-once -> auto-const */
        "var r = flag ? 1 : 2; assert(r == 2);" } },
    { "decl-vs-ternary: a bare ternary statement (id condition)",
      { "var dyn flag = runtime(true); var a = 1; var b = 2;",
        /* `flag ? a : b` at statement start is a ternary, not a `flag? a`
         * nullable-struct decl (the lookahead bails on the `:`). */
        "flag ? a : b; assert(true);" } },
    { "decl-vs-ternary: a nullable typed decl still parses",
      { "int? x; assert(x == none); int? y = 5; assert(y == 5);" } },
    { "ternary in a subscript is a subscript; a slice bound needs parens",
      { "var a = [10, 20, 30];",
        "assert(a[1 > 0 ? 0 : 2] == 10);",
        "assert(a[(1 > 0 ? 0 : 1):3] == [10, 20, 30]);" } },
    { "ternary as an argument and a nested ternary",
      { "func id(x) => x;",
        "var n = 2;",
    ...
    /* Native string foreach (StrLen + LoadStrChar counted loop): single-var
     * binds each char (a fresh 1-char string), indexed binds index+char; empty
     * / break / continue behave. Runs on BOTH engines (the differential). */
    { "foreach over a string iterates its chars",
      { "var r = \"\";",
        "foreach (var c in \"abc\") { r = r + c + \"-\"; }",
        "assert(r == \"a-b-c-\");" } },
    { "indexed foreach over a string binds index + char",
      { "var r = \"\";",
        "foreach (var i, c in indexed \"xy\") { r = r + str(i) + c; }",
        "assert(r == \"0x1y\");" } },
    { "string foreach: empty string, break, continue",
      { "var n = 0; foreach (var c in \"\") { n = n + 1; }",
        "assert(n == 0);",
        "var r = \"\";",
     ...
     */
    {
        ...
        {
            "assert(array_storage(range(5)) == \"int\");",
            "assert(array_storage(range(0, 10, 2)) == \"int\");",
            "assert(array_storage(array(4, 0)) == \"int\");",
            "assert(array_storage(array(4, 1.5)) == \"float\");",
            "assert(array_storage(array(4, \"x\")) == \"general\");",
            "assert(array_storage(array(4)) == \"general\");",
            "assert(array_storage([1,2,3]) == \"int\");",
            "assert(array_storage([1.0,2.0]) == \"float\");",
            "assert(array_storage([1,\"x\"]) == \"general\");",
            "assert(array(4, 0) == [0,0,0,0]);",
            "assert(array(3, 2) == [2,2,2]);",
            "assert(array_storage(make_array(5, func(i) => i*i)) == \"int\");",
    ...
    },
    {
        ...
        "Typed arrays: keys()/values() of a scalar dict are flat",
        {
            ...
            "assert(array_storage(ki) == \"int\");",
            "assert(array_storage(vi) == \"int\");",
    ...
    },
    {
        ...
        "Typed arrays: flat += concatenation, clone, intptr (float/bool)",
        {
            ...
            "assert(array_storage(fc) == \"float\"); assert(len(fc) == 3);",
            "var bc = clone([true]); assert(array_storage(bc) == \"bool\");",
      ...
      { "var dyn a = [1.0]; a += 5;" }, &typeid(TypeErrorEx) },
    {
        ...
        "Typed arrays: flat float/bool builtin operations",
        {
            ...
            "append(b, true); assert(array_storage(b) == \"bool\");",
            "assert(sum(b) == 2);",
    ...
    },
    {
        "Typed arrays: flat subscript store + compound keep storage flat",
        {
            ...
            "assert(array_storage(a) == \"int\");",
            "a[0] += 100;",
            "assert(a[0] == 100 && array_storage(a) == \"int\");",
            "var f = array(3, 0.0);",
            "f[1] = 2.5; f[0] += 1;",
            "assert(array_storage(f) == \"float\");",
        },
    ...
    },
    {
        "Typed arrays: flat reads (sum/min/max/reverse/sort/foreach)",
        {
            ...
            "var b = range(5);",
            "reverse(b);",
            "assert(array_storage(b) == \"int\" && b == [4,3,2,1,0]);",
            "sort(b);",
            "assert(array_storage(b) == \"int\" && b == [0,1,2,3,4]);",
            "var t = 0;",
    ...
    },
    {
        "Typed arrays: slices share flat storage, COW on write, clones flat",
        {
            "var a = range(5);",
            "var s = a[1:4];",
            "assert(array_storage(s) == \"int\" && s == [1,2,3]);",
            "a[1] = 100;",
            "assert(s == [1,2,3]);",
            "assert(a == [0,100,2,3,4]);",
            "var c = clone(range(4));",
            "assert(array_storage(c) == \"int\");",
            "var d = deepclone(range(4));",
            "assert(array_storage(d) == \"int\");",
        },
    },
    {
...

    {
        ...
        {
            "assert(kindstr(true) == \"bool\");",
            "assert(kindstr(false) == \"bool\");",
            "assert(kindstr(1 < 2) == \"bool\");",
            "assert(kindstr(1 == 1) == \"bool\");",
            "assert(kindstr(!0) == \"bool\");",
            "assert(kindstr(true && false) == \"bool\");",
            "assert(kindstr(1) == \"int\");",         /* 1 stays int */
            "var b = true; assert(kindstr(b) == \"bool\");",
        },
    },
    {
    ...
    },
    {
        "bool: promotion chain bool <= int <= float",
        {
            ...
            "assert(true * 10 == 10);",
            "assert(true + 2.5 == 3.5);",
            "assert(kindstr(true + 1) == \"int\");",
            "assert(kindstr(true + 2.5) == \"float\");",
            "assert(-true == -1);",
            "assert(kindstr(-true) == \"int\");",
        },
    },
    {
    ...
    },
    {
        "array<bool>: clone keeps flat, const is deep read-only",
        {
            "var b = [true, false];",
            "var c = clone(b);",
            "assert(array_storage(c) == \"bool\");",
            "c[0] = false;",
            "assert(b == [true, false]);",
            "const K = [true, false, true];",
            "assert(array_storage(K) == \"bool\");",
        },
    },
    {
    ...
    { "named args: a typed param coerces (float <- int) when named",
      { "func f(float a, int b) => a;",
        "assert(kindstr(f(a: 3, b: 2)) == \"float\");",
    ...
    { "struct: a var holds the struct type (not dyn)",
      { "struct Point { int x; int y; }",
        "var p = Point(1, 2); assert(kindstr(p) == \"struct\");" } },
    ...
    { "struct: a struct flowing through a dyn var",
      { "struct Point { int x; int y; } var dyn d = Point(5, 6);",
         ...
         * compile-time query); the struct still works at runtime. */
        "assert(d.x == 5); assert(kindstr(d) == \"dyn\");" } },
    ...
    /* ---- explicit struct-typed variable declarations ---- */
    { "struct: explicit type annotation on a variable",
      { "struct A { int a; }",
        "A x = A(10); assert(x.a == 10);" } },
    { "struct: an uninitialized struct var zero-inits recursively",
      { "struct P { int x; int y; }",
        "struct L { P a; array pts; str name; }",
        "L v;",
        "assert(v.a.x == 0); assert(v.a.y == 0);",
        "assert(len(v.pts) == 0); assert(v.name == \"\");" } },
    { "struct: a struct annotation pins the type (wrong struct -> error)",
      { "struct A { int a; } struct B { int b; }",
        "A x = B(1);" }, &typeid(TypeMismatchEx) },
    { "struct: reassigning a wrong struct type is an error",
      { "struct A { int a; } struct B { int b; }",
        "A x = A(1); x = B(2);" }, &typeid(TypeMismatchEx) },
    { "struct: 'opt' struct annotation defaults to none",
    ...
    { "struct array: empty + append stays flat (build-up pattern)",
      { "struct P { int x; int y; }",
        ...
        "assert(array_storage(a) == \"struct\"); assert(len(a) == 4);",
    ...
    { "struct build: append(a, S(..)) constructs in place, stays flat",
      { "struct P { int x; int y; }",
        ...
        "assert(array_storage(a) == \"struct\"); assert(len(a) == 6);",
    ...
    { "struct build: a nested-POD constructor appends in place",
      { "struct P { int x; int y; } struct L { P a; P b; }",
        ...
        "assert(array_storage(ls) == \"struct\");" } },
    ...
    { "struct build: appending a plain struct value still works",
      { "struct P { int x; }",
        ...
        "assert(a[0].x == 7); assert(array_storage(a) == \"struct\");" } },
    ...
    { "types: scalar annotations declare and check",
      { "int x = 32; float f = 1.5; str s = \"hi\"; bool b = true;",
        "assert(x == 32); assert(kindstr(x) == \"int\");",
        "assert(f == 1.5); assert(kindstr(f) == \"float\");",
        "assert(s == \"hi\"); assert(kindstr(s) == \"str\");",
        "assert(b == true); assert(kindstr(b) == \"bool\");" } },
    { "types: array/dict annotations (generic, element inferred)",
      { "array a = [1,2,3]; dict d = {\"k\": 1};",
        "assert(a == [1,2,3]); assert(kindstr(a) == \"array\");",
        "assert(array_storage(a) == \"int\");",   /* element type kept */
        "assert(d[\"k\"] == 1); assert(kindstr(d) == \"dict\");" } },
    { "types: float annotation coerces an int initializer to float",
      { "float f = 3; assert(f == 3.0); assert(kindstr(f) == \"float\");" } },
    { "types: float coercion on reassignment too",
      { "float f = 1.0; f = 5; assert(f == 5.0);",
        "assert(kindstr(f) == \"float\");" } },
    { "types: int annotation coerces a bool to int",
      { "int x = true; assert(x == 1); assert(kindstr(x) == \"int\");" } },
    ...
    { "types: const scalar annotation (coerced + inlined)",
      { "const float c = 3; assert(c == 3.0); assert(kindstr(c) == \"float\");",
    ...
    /* a non-opt explicitly-typed variable can never become none */
    { "types reject: none into a non-opt typed var (decl)",
      { "int a = none;" }, &typeid(NullabilityEx) },
    { "types reject: none into a non-opt typed var (reassign)",
      { "int a = 5; a = none;" }, &typeid(NullabilityEx) },
    { "types reject: none into a non-opt struct var",
      { "struct P { int x; } P p = P(1); p = none;" },
      &typeid(NullabilityEx) },
    { "types: opt typed var accepts none; plain var is implicitly nullable",
      { "int? a = none; assert(a == none);",
        "opt int b; b = none; assert(b == none);",
        "var x; x = none; assert(x == none);" } },

    /* decltype(var): the static (declared or inferred) type of a VARIABLE */
    { "decltype: annotated scalar and opt",
      { "int x = 5; assert(decltype(x).name == \"int\");",
        "int? a; assert(decltype(a).name == \"int?\");",
        "dyn? d; assert(decltype(d).name == \"dyn?\");" } },
    { "decltype: inferred container types",
      { "var a = [1, 2, 3]; assert(decltype(a).name == \"array<int>\");",
        "var m = {\"k\": 1};",
        "assert(decltype(m).name == \"dict<str,int>\");" } },
    { "decltype: a struct variable",
      { "struct P { int x; } P p = P(1);",
        "assert(decltype(p).name == \"P\");" } },
    { "decltype / type / kindstr of an int? var are all its STATIC type",
      { "int? a;",
        "assert(decltype(a).name == \"int?\");",
        "assert(type(a).name == \"int?\"); assert(type(a).nullable);",
        "assert(kindstr(a) == \"int\");" } },
    { "decltype reject: a non-identifier argument",
      { "var z = decltype(5);" }, &typeid(TypeMismatchEx) },
    { "decltype reject: an expression argument",
      { "int a = 1; var z = decltype(a + 1);" }, &typeid(TypeMismatchEx) },
    { "decltype reject: wrong arg count",
      { "int a = 1; var z = decltype(a, a);" }, &typeid(WrongArgCountEx) },
    ...
    /* type() / decltype() -> a Type reflection object (native composite). The
     * recursive elem/key/val are `opt Type`, so reading a field needs an
     * if-narrow (assert does not narrow). */
    { "type: returns a Type object with kind/name/nullable",
      { "var a = [1, 2, 3]; var t = type(a);",
        "assert(t.kind == \"array\"); assert(t.name == \"array<int>\");",
        "assert(t.nullable == false);",
        "var e = t.elem; assert(e != none);",
        "if (e != none) { assert(e.kind == \"int\"); }" } },
    { "type: a dict Type carries key and val (recursive)",
      { "var m = {\"a\": 1}; var t = type(m); assert(t.kind == \"dict\");",
        "var k = t.key; var v = t.val; assert(k != none); assert(v != none);",
        "if (k != none) { assert(k.kind == \"str\"); }",
        "if (v != none) { assert(v.kind == \"int\"); }" } },
    { "type: a scalar Type has no elem/key/val",
      { "var t = type(5); assert(t.kind == \"int\");",
        "assert(t.elem == none); assert(t.key == none); assert(t.val == none);"
        } },
    { "decltype: returns a Type object of the variable's static type",
      { "int? a; var t = decltype(a);",
        "assert(t.kind == \"int\"); assert(t.name == \"int?\");",
        "assert(t.nullable);" } },
    { "type: nested array<array<int>> Type (recursive elem)",
      { "var g = [[1, 2], [3]]; var e = type(g).elem; assert(e != none);",
        "if (e != none) { assert(e.name == \"array<int>\");",
        "  var ee = e.elem; assert(ee != none);",
        "  if (ee != none) { assert(ee.kind == \"int\"); } }" } },

    /* ---- parameterized container types: array<T> / dict<K, V> ---- */
    { "array<T>: a typed array is flat and decltype-reported",
      { "array<int> a = [1, 2, 3];",
        "assert(array_storage(a) == \"int\");",
        "assert(decltype(a).name == \"array<int>\");" } },
    { "array<T>: an empty typed array starts flat",
      { "array<int> a; append(a, 5);",
        "assert(array_storage(a) == \"int\"); assert(a[0] == 5);" } },
    { "array<T>: empty float / bool typed arrays are flat",
      { "array<float> f; array<bool> b;",
        "assert(array_storage(f) == \"float\");",
        "assert(array_storage(b) == \"bool\");" } },
    { "dict<K,V>: a typed dict",
      { "dict<str, int> m = {\"a\": 1};",
        "assert(m[\"a\"] == 1);",
        "assert(decltype(m).name == \"dict<str,int>\");" } },
    { "array<T>: nested array<array<int>>",
      { "array<array<int>> g = [[1, 2], [3]];",
        "assert(g[0][1] == 2);",
        "assert(decltype(g).name == \"array<array<int>>\");" } },
    { "container types: deep nesting (the >>> token split)",
      { "array<array<array<int>>> a = [[[7]]]; assert(a[0][0][0] == 7);" } },
    { "dict<K, array<T>>: composed key/value types",
      { "dict<int, array<str>> m = {1: [\"a\", \"b\"]};",
        "assert(m[1][0] == \"a\");",
        "assert(decltype(m).name == \"dict<int,array<str>>\");" } },
    { "array<T>: a flat POD-struct array via the annotation",
      { "struct P { int x; } array<P> a = [P(1), P(2)];",
        "assert(a[1].x == 2); assert(array_storage(a) == \"struct\");" } },

    { "array<T> reject: wrong element type (decl)",
      { "array<int> a = [\"x\"];" }, &typeid(TypeMismatchEx) },
    { "array<T> reject: wrong element type (reassign)",
      { "array<int> a = [1]; a = [\"x\"];" }, &typeid(TypeMismatchEx) },
    { "dict<K,V> reject: wrong value type",
      { "dict<str, int> m = {\"a\": \"b\"};" }, &typeid(TypeMismatchEx) },
    { "array<T> reject: a dyn-laundered non-fitting element",
      { "array<int> a = [1]; append(a, \"x\");" },
      &typeid(TypeMismatchEx) },

    { "array<T>: non-opt defaults to [] and rejects none; opt accepts it",
      { "array<int> a; assert(a == []);",
        "array<int>? b; assert(b == none);",
        "array<int>? c = [1]; c = none; assert(c == none);" } },
    { "array<T> reject: none into a non-opt typed array",
      { "array<int> a = none;" }, &typeid(NullabilityEx) },
    { "dict<K,V> reject: none into a non-opt typed dict",
      { "dict<str, int> m = none;" }, &typeid(NullabilityEx) },

    { "array<T>: a parameterized-container function parameter",
      { "func sum(array<int> xs) {",
        "  var s = 0; foreach (var x in xs) s += x; return s; }",
        "assert(sum([1, 2, 3, 4]) == 10);" } },
    { "array<T> reject: wrong element type as a param argument",
      { "func f(array<int> xs) => len(xs); f([\"a\"]);" },
      &typeid(TypeMismatchEx) },

    { "array<T>: a struct field with a parameterized container",
      { "struct S { array<int> xs; dict<str, int> m; }",
        "var s = S([1, 2, 3], {\"a\": 1});",
        "assert(s.xs[2] == 3); assert(s.m[\"a\"] == 1);" } },
    { "array<T> reject: wrong element type for a struct field",
      { "struct S { array<int> xs; } var s = S([\"x\"]);" },
      &typeid(TypeMismatchEx) },

    { "array<T>: a const parameterized array",
      { "const array<int> a = [1, 2, 3]; assert(a[1] == 2);",
        "assert(decltype(a).name == \"array<int>\");" } },
    ...
    { "types: typed parameters declare and check",
      { "func add(int a, float b) => a + b;",
        "assert(add(2, 3) == 5.0);",
        "assert(kindstr(add(2, 3)) == \"float\");" } },
    { "types: typed param coerces a widening argument",
      { "func f(float x) => x; assert(f(3) == 3.0); assert(kindstr(f(3)) ==",
        "\"float\");" } },
    ...
    { "syntax: ? is the canonical nullable suffix (var?/int?/dyn?/array?)",
      { "int? a = 5; assert(a == 5); assert(kindstr(a) == \"int\");",
        "var? b; assert(b == none);",
        "dyn? c; assert(c == none);",
    ...
    { "reflect: layout() returns a structured StructLayout (POD)",
      { "struct V { int a; float b; }",
        "var lay = layout(V);",
        "assert(lay.name == \"V\"); assert(lay.pod);",
        "assert(lay.size == 16); assert(len(lay.fields) == 2);",
        "assert(lay.fields[0].name == \"a\");",
        "assert(lay.fields[0].type == \"int\");",
        "assert(lay.fields[0].offset == 0);",
        "assert(lay.fields[1].name == \"b\");",
        "assert(lay.fields[1].offset == 8);" } },
    { "reflect: layout() of a boxed struct (offsets are -1)",
      { "struct N { int v; dyn? next; }",
        "var lay = layout(N);",
        "assert(lay.name == \"N\"); assert(lay.pod == false);",
        "assert(len(lay.fields) == 2);",
        "assert(lay.fields[1].name == \"next\");",
        "assert(lay.fields[0].offset == -1);" } },
    { "reflect: layout() on a struct instance and field types",
      { "struct G { array<int> xs; }",
        "var g = G([1, 2]); var lay = layout(g);",
        "assert(lay.name == \"G\");",
        "assert(lay.fields[0].type == \"array<int>\");" } },
    ...
    /*
     * ---- catchd: the CATCH-DISPATCH battery (task #78) ----
     *
     * Written BEFORE the catch/reraise dispatch redesign (the matcher moving
     * from interpreted CatchTest ops into the raise path, driven by a
     * per-chunk handler table), pinning the semantics the redesign must
     * preserve byte-for-byte. Every entry runs on BOTH engines (the
     * differential), and each targets a specific dispatch-order mechanic:
     * clause selection, the no-match paths, the finally-reraise routing,
     * binding, and the boundary entries. Do not "simplify" these - each
     * shape exists to catch a specific regression.
     */
    { "catchd: first clause matches - later clauses and catch-all skipped",
      { "struct A { int v; } struct B { int v; }",
        "var r = 0;",
        "try { throw A(1); }",
        "catch (A as e) { r = 10 + e.v; }",
        "catch (B as e) { r = 20 + e.v; }",
        "catch { r = 99; }",
        "assert(r == 11);" } },
    { "catchd: middle clause matches",
      { "struct A { int v; } struct B { int v; } struct C { int v; }",
        "var r = 0;",
        "try { throw B(2); }",
        "catch (A) { r = 1; } catch (B as e) { r = 20 + e.v; }",
        "catch (C) { r = 3; }",
        "assert(r == 22);" } },
    { "catchd: last clause (catch-all) matches",
      { "struct A { int v; } struct B { int v; }",
        "var r = 0;",
        "try { throw runtime(6) / 0; }",
        "catch (A) { r = 1; } catch (B) { r = 2; } catch { r = 3; }",
        "assert(r == 3);" } },
    { "catchd: a multi-type clause matches its SECOND listed type",
      { "struct A { int v; } struct B { int v; }",
        "var r = 0;",
        "try { throw B(7); } catch (A, B as e) { r = e.v; }",
        "assert(r == 7);" } },
    { "catchd: `catch (e)` is a TYPE match on the name `e`, NOT a bind-all",
      /* the catch-all is the bare `catch` (parenless) and cannot bind; a
       * parenthesized name is always a TYPE list - so `catch (e)` catches
       * only an exception NAMED `e`. Pin both readings. */
      { "struct e { int v; }",
        "struct A { int v; }",
        "var trc = [];",
        "try { throw e(5); } catch (e as x) { append(trc, x.v); }",
        "try {",
        "  try { throw A(1); } catch (e) { append(trc, -1); }",
        "} catch (A) { append(trc, 2); }",
        "assert(trc == [5, 2]);" } },
    { "catchd: the bare catch-all catches anything (no binding)",
      { "struct A { int v; }",
        "var trc = [];",
        "try { throw A(1); } catch { append(trc, 1); }",
        "try { var dyn z = runtime(1) / 0; } catch { append(trc, 2); }",
        "assert(trc == [1, 2]);" } },
    { "catchd: no clause matches - the OUTER try in the SAME frame catches",
      { "struct A { int v; } struct B { int v; }",
        "var r = 0;",
        "try {",
        "  try { throw B(3); } catch (A) { r = 1; }",
        "} catch (B as e) { r = 30 + e.v; }",
        "assert(r == 33);" } },
    { "catchd: three nested trys - the MIDDLE one matches",
      { "struct X { int v; }",
        "var trc = [];",
        "try {",
        "  try {",
        "    try { throw X(1); } catch (DivisionByZeroEx) { append(trc, 1); }",
        "  } catch (X) { append(trc, 2); }",
        "} catch { append(trc, 3); }",
        "assert(trc == [2]);" } },
    { "catchd: no match in the throwing frame - the CALLER's clause matches",
      { "struct A { int v; } struct B { int v; }",
        "func inner(int n) {",
        "  try { throw B(n); } catch (A) { return -1; }",
        "  return -2;",
        "}",
        "var r = 0;",
        "try { inner(4); } catch (B as e) { r = e.v; }",
        "assert(r == 4);" } },
    { "catchd: the walk crosses THREE frames to the matching clause",
      { "struct E { int v; }",
        "func f3(int n) { throw E(n); }",
        "func f2(int n) { return f3(n) + 1; }",
        "func f1(int n) { return f2(n) + 1; }",
        "var r = 0;",
        "try { f1(9); } catch (E as e) { r = e.v; }",
        "assert(r == 9);" } },
    { "catchd: each frame's non-matching handlers pop as the walk passes",
      { "struct A { int v; } struct E { int v; }",
        "var trc = [];",
        "func f2(int n) {",
        "  try { throw E(n); } catch (A) { append(trc, -2); }",
        "  append(trc, -3);",
        "}",
        "func f1(int n) {",
        "  try { f2(n); } catch (A) { append(trc, -1); }",
        "  append(trc, -4);",
        "}",
        "try { f1(5); } catch (E as e) { append(trc, e.v); }",
        "assert(trc == [5]);" } },
    { "catchd: finally runs then the exception RERAISES to the outer catch",
      { "struct E { int v; }",
        "var trc = [];",
        "try {",
        "  try { throw E(1); }",
        "  catch (DivisionByZeroEx) { append(trc, -1); }",
        "  finally { append(trc, 10); }",
        "} catch (E) { append(trc, 20); }",
        "assert(trc == [10, 20]);" } },
    { "catchd: finally reraise crosses a FRAME to the caller's catch",
      { "struct E { int v; }",
        "var trc = [];",
        "func f(int n) {",
        "  try { throw E(n); } finally { append(trc, 1); }",
        "}",
        "try { f(2); } catch (E as e) { append(trc, 100 + e.v); }",
        "assert(trc == [1, 102]);" } },
    { "catchd: a throw INSIDE finally replaces the pending reraise",
      { "struct A { int v; } struct B { int v; }",
        "var r = 0;",
        "try {",
        "  try { throw A(1); } finally { throw B(2); }",
        "} catch (A) { r = -1; } catch (B as e) { r = e.v; }",
        "assert(r == 2);" } },
    { "catchd: caught in this frame, then finally, then normal fall-through",
      { "struct E { int v; }",
        "var trc = [];",
        "try { throw E(1); }",
        "catch (E) { append(trc, 1); }",
        "finally { append(trc, 2); }",
        "append(trc, 3);",
        "assert(trc == [1, 2, 3]);" } },
    { "catchd: rethrow reaches the outer clause of the SAME frame",
      { "struct E { int v; }",
        "var trc = [];",
        "try {",
        ...
        "assert(trc == [1, 8]);" } },
    { "catchd: rethrow crosses a frame; the binding still carried the payload",
      { "struct E { int v; }",
        "func f(int n) {",
        "  try { throw E(n); } catch (E as e) { rethrow; }",
        "  return -1;",
        "}",
        "var r = 0;",
        "try { f(6); } catch (E as e) { r = e.v; }",
        "assert(r == 6);" } },
    { "catchd: a NEW throw from a catch body reaches the outer clause",
      { "struct A { int v; } struct B { int v; }",
        "var r = 0;",
        "try {",
        "  try { throw A(1); } catch (A) { throw B(2); }",
        "} catch (B as e) { r = e.v; }",
        "assert(r == 2);" } },
    { "catchd: the bind slot is an ordinary var after the catch body",
      { "struct E { int v; }",
        "var r = 0;",
        "try { throw E(3); } catch (E as e) { r = e.v; }",
        "r += 100;",
        "assert(r == 103);" } },
    { "catchd: a builtin-thrown error (OOB) dispatches like a throw",
      { "var a = [1, 2, 3];",
        "var r = 0;",
        "try { r = a[runtime(9)]; } catch (OutOfBoundsEx) { r = -1; }",
        "assert(r == -1);" } },
    { "catchd: an exception from a COMPARATOR callback crosses the builtin",
      { "struct E { int v; }",
        "var a = [3, 1, 2];",
        "var r = 0;",
        "try { sort(a, func(x, y) { throw E(4); }); }",
        "catch (E as e) { r = e.v; }",
        "assert(r == 4);" } },
    { "catchd: a user struct NAMED like a builtin matches both ways",
      /* the #74 canonical-intern case: `catch (TypeErrorEx)` must catch BOTH
       * the builtin and a user struct of that name (they share one interned
       * name; the catch matches by name, deliberately) */
      { "struct TypeErrorEx { int v; }",
        "var r = 0;",
        "try { throw TypeErrorEx(5); } catch (TypeErrorEx as e) { r = e.v; }",
        "assert(r == 5);" } },
    { "catchd: try/catch inside a CATCH body",
      { "struct A { int v; } struct B { int v; }",
        "var trc = [];",
        "try { throw A(1); }",
        "catch (A) {",
        "  append(trc, 1);",
        "  try { throw B(2); } catch (B) { append(trc, 2); }",
        "  append(trc, 3);",
        "}",
        "assert(trc == [1, 2, 3]);" } },
    /*
     * The nested pend/exc CLOBBER class (#78 step 2). One shared per-record
     * {exc, pend} pair served every try region of a frame, so same-frame
     * NESTING clobbered it (the tree-walker was immune - its state lives in
     * TryCatchStmt::do_eval C++ locals, nested by recursion). Fixed by the
     * per-try-REGION slots (VmPendState, vm.cpp); each test below was a
     * live VM divergence - or a silent exception LOSS - before the fix.
     */
    { "catchd: try/catch inside a FINALLY body (pending exc survives)",
      { "struct A { int v; } struct B { int v; }",
        "var trc = [];",
        "try {",
        "  try { throw A(1); }",
        "  finally {",
        "    try { throw B(2); } catch (B) { append(trc, 1); }",
        "    append(trc, 2);",
        "  }",
        "} catch (A) { append(trc, 3); }",
        "assert(trc == [1, 2, 3]);" } },
    { "catchd: try/FINALLY inside a finally (pending action survives)",
      /* before the fix the inner SetPend(normal) clobbered pend=reraise and
       * the exception was silently LOST - the worst of the class */
      { "struct A { int v; }",
        "var trc = [];",
        "try {",
        "  try { throw A(1); }",
        "  finally {",
        "    try { append(trc, 1); } finally { append(trc, 2); }",
        "    append(trc, 3);",
        "  }",
        "} catch (A) { append(trc, 4); }",
        "assert(trc == [1, 2, 3, 4]);" } },
    { "catchd: a caught inner try does not corrupt the outer rethrow",
      { "struct A { int v; } struct B { int v; }",
        "var trc = [];",
        "try {",
        "  try { throw A(1); }",
        ...
        "    rethrow;",
        "  }",
        "} catch (A as e) { append(trc, 100 + e.v); }",
        "  catch (B as e) { append(trc, 200 + e.v); }",
        "assert(trc == [1, 101]);" } },
    { "catchd: three-deep region nesting, each level keeps its own state",
      { "struct A { int v; } struct B { int v; } struct C { int v; }",
        "var trc = [];",
        "try {",
        "  try { throw A(1); }",
        ...
        "    try { throw B(2); }",
        "    finally {",
        "      try { throw C(3); } catch (C) { append(trc, 3); }",
        "    }",
        "  }",
        "} catch (A) { append(trc, -1); } catch (B) { append(trc, 2); }",
        "assert(trc == [3, 2]);" } },
    { "catchd: sequential finallys inside one finally, reraise preserved",
      { "struct A { int v; }",
        "var trc = [];",
        "try {",
        "  try { throw A(9); }",
        "  finally {",
        "    try { append(trc, 1); } finally { append(trc, 2); }",
        "    try { append(trc, 3); } finally { append(trc, 4); }",
        "  }",
        "} catch (A as e) { append(trc, e.v); }",
        "assert(trc == [1, 2, 3, 4, 9]);" } },
    { "catchd: rethrow from a catch inside another catch (regions nested)",
      { "struct A { int v; } struct B { int v; }",
        "var trc = [];",
        "try {",
        "  try { throw A(1); }",
        "  catch (A) {",
        "    try { throw B(2); }",
        "    catch (B) { append(trc, 1); rethrow; }",
        "  }",
        "} catch (B as e) { append(trc, 10 + e.v); }",
        "  catch (A as e) { append(trc, 100 + e.v); }",
        "assert(trc == [1, 12]);" } },
    { "catchd: the pend slot resets on region re-entry (loop over finally)",
      /* an ABANDONED reraise (break inside the finally) must not leak into
       * the region's next cycle */
      { "struct A { int v; }",
        "var trc = [];",
        "for (var i = 0; i < 3; i++) {",
        "  try {",
        "    try { if (i == 0) { throw A(i); } append(trc, 10 + i); }",
        "    finally { if (i == 0) { append(trc, i); break; } }",
        "  } catch (A) { append(trc, -1); }",
        "}",
        "assert(trc == [0]);" } },
    { "catchd: a flow op INSIDE the finally body wins over a pending throw",
      /* the tree-walker spec (pinned on both engines): the finally body runs
       * with the pending action suspended, and a flow signal raised INSIDE
       * it REPLACES that action - here the `return` wins and the exception
       * is abandoned. This shape also HUNG the JIT until the deleted-
       * handler-pc entry-stub fix (a throw dispatching into a deleted
       * finally-only region resumed at the run head and looped forever). */
      { "struct A { int v; }",
        "func f() { try { throw A(1); } finally { return 5; } return 9; }",
        "assert(f() == 5);" } },
    { "catchd: `break` inside the finally body abandons the pending throw",
      /* the second JIT-hang shape */
      { "struct A { int v; }",
        "var trc = [];",
        "for (var i = 0; i < 3; i++) {",
        "  try { throw A(i); } finally { append(trc, i); break; }",
        "}",
        "assert(trc == [0]);" } },
    { "catchd: a `return` in the finally replaces the try body's return",
      { "func f() { try { return 1; } finally { return 2; } }",
        "assert(f() == 2);" } },
    { "catchd: handler push/pop balance across many loop iterations",
      { "struct E { int v; }",
        "var c = 0;",
        "for (var i = 0; i < 200; i++) {",
        "  try {",
        ...
        "}",
        "assert(c == 133 + 6700);" } },
    { "catchd: a throw from DEEP recursion reaches the top handler",
      { "struct E { int v; }",
        "func down(int n) {",
        "  if (n == 0) { throw E(42); }",
        "  return down(n - 1) + 1;",
        "}",
        "var r = 0;",
        "try { down(60); } catch (E as e) { r = e.v; }",
        "assert(r == 42);" } },
    { "catchd: sequential trys in one frame - each pops before the next",
      { "struct A { int v; } struct B { int v; }",
        "var trc = [];",
        "try { throw A(1); } catch (A) { append(trc, 1); }",
        "try { throw B(2); } catch (B) { append(trc, 2); }",
        "try { append(trc, 3); } catch { append(trc, -1); }",
        "assert(trc == [1, 2, 3]);" } },
    { "catchd: no clause anywhere - the error escapes (uncaught)",
      { "struct A { int v; } struct B { int v; }",
        "try { throw B(1); } catch (A) { print(0); }" },
      &typeid(ExceptionObject) },
    { "catchd: unmatched through a finally - still escapes after it runs",
      { "struct A { int v; } struct B { int v; }",
        "var g = 0;",
        "func f() {",
        "  try { throw B(1); } catch (A) { print(0); } finally { g = 7; }",
        "}",
        "f();" },
      &typeid(ExceptionObject) },
    { "catchd: a caught builtin exception object can be RE-thrown",
      { "var trc = [];",
        "try {",
        "  try { var dyn z = runtime(1) / 0; }",
        "  catch (DivisionByZeroEx as e) { append(trc, 1); throw e; }",
        "} catch (DivisionByZeroEx) { append(trc, 2); }",
        "assert(trc == [1, 2]);" } },
    { "catchd: throw from an INLINED body inside a try - caught with frames",
      { "struct E { int v; }",
        "func thrower(dyn x) => x / 0;",
        "func wrap(dyn x) => thrower(x) + 1;",
        "var r = 0;",
        "try { wrap(runtime(3)); } catch (DivisionByZeroEx) { r = 1; }",
        "assert(r == 1);" } },
};
...
static bool static_type_assignable_rules()
{
    ...
    StaticTypeRef d = a.dyn_ty(), n = a.none_ty();

    if (!static_type_assignable(i, i))            return false;
    if (!static_type_assignable(i, f))            return false;  /* int ->
        float */
    if ( static_type_assignable(f, i))            return false;  /* float -/->
        int */
    if (!static_type_assignable(i, d))            return false;  /* any -> dyn
        */
    if ( static_type_assignable(d, i))            return false;  /* dyn -/->
        int */
    if ( static_type_assignable(s, i))            return false;  /* str -/->
        int */
    if ( static_type_assignable(n, i))            return false;  /* none -/->
        int */
    if (!static_type_assignable(n, a.int_ty(true)))          return false; /*
        ->opt */
    if ( static_type_assignable(a.int_ty(true), i))          return false; /*
        opt-/->*/
    if (!static_type_assignable(i, a.int_ty(true)))          return false;
    if (!static_type_assignable(i, a.float_ty(true)))        return false; /*
        promote*/
    return true;
}
...
static bool static_type_join_rules()
{
    StaticTypeArena a;
    StaticTypeRef i = a.int_ty(), f = a.float_ty(), s = a.str_ty();
    StaticTypeRef n = a.none_ty(), d = a.dyn_ty();
    ...
    StaticTypeRef ai = a.array_of(i), as = a.array_of(s);
    if (!static_type_equal(a.join(ai, ai), ai))     return false;

    StaticTypeRef mixed = a.join(ai, as);                         /* ->
        array<dyn> */
    if (!mixed || mixed->kind != StaticTypeKind::Array)           return false;
    if (static_type_resolve(mixed->elem)->kind != StaticTypeKind::Dyn)
        return false;
    return true;
}
...
static bool static_type_unify_vars()
{
    ...
    StaticTypeRef v = a.fresh_var();
    if (!static_type_unify(v, a.int_ty()))          return false;
    if (static_type_resolve(v) != a.int_ty())       return false;  /* v bound
        int */
    if (static_type_unify(v, a.str_ty()))           return false;  /* int !=
        str */

    /* occurs-check: w := array<w> is an infinite type and must be rejected */
    StaticTypeRef w = a.fresh_var();
    if (static_type_unify(w, a.array_of(w)))        return false;

    /* structural unify binds the inner variable */
    StaticTypeRef x = a.fresh_var();
    if (!static_type_unify(a.array_of(x), a.array_of(a.int_ty()))) return false;
    if (static_type_resolve(x) != a.int_ty())       return false;
    return true;
}
...
static bool static_type_to_string_basic()
{
    ...
    if (static_type_to_string(a.int_ty()) != "int")
        return false;
    if (static_type_to_string(a.int_ty(true)) != "int?")
        return false;
    if (static_type_to_string(a.array_of(a.str_ty())) != "array<str>")
        return false;
    if (static_type_to_string(a.none_ty()) != "none")
        return false;
    if (static_type_to_string(a.dyn_ty()) != "dyn")
        return false;
    if (static_type_to_string(a.with_opt(a.dyn_ty(), true)) != "dyn?")
        return false;

    /* `?` is a suffix that composes at every level */
    if (static_type_to_string(a.array_of(a.int_ty(true))) != "array<int?>")
        return false;
    if (static_type_to_string(a.array_of(a.int_ty(), true)) != "array<int>?")
        return false;

    StaticTypeRef fn = a.func_of({ a.int_ty(), a.str_ty() },
                          { false, true }, a.float_ty());
    if (static_type_to_string(fn) != "func(int,str?)->float")
        return false;
    return true;
}
...
static const std::vector<repl_test> repl_tests =
{
    ...
    { "the real optimizers run per input (flat array storage)",
      { { "var fa = [1, 2, 3]", "=> [1, 2, 3]" },
        { "array_storage(fa)", "=> \"int\"" },
        { "var fb = [1.5, 2.5]", "" },
        { "array_storage(fb)", "=> \"float\"" } } },
    ...
    { "decltype reports a variable's static type across inputs",
      { { "int? a;", "" },
        ...
        { "type(a).kind", "=> \"int\"" },
        { "var b = [1, 2, 3]", "" },
        { "decltype(b).name", "=> \"array<int>\"" } } },
    { "a non-opt typed global cannot be assigned none in the REPL",
      { { "int c = 5", "=> 5" },
        { "c = none", "cannot be none" } } },
    { "ternary / ?? work and const-fold across REPL inputs",
      { { "const FLAG = true;", "" },
        { "int? g;", "" },
    ...
    { "parameterized container types persist and check across inputs",
      { { "array<int> g = [10, 20]", "[10, 20]" },
        { "decltype(g).name", "=> \"array<int>\"" },
        { "func f(array<int> xs) => len(xs)", "" },
        { "f([1, 2, 3])", "=> 3" },
        { "f([\"a\"])", "but the parameter is 'array<int>'" } } },
...
struct VmOpCounts {
    ...
    size_t checkfunc = 0, mapfilter = 0;
...
static void count_chunk_ops(const Chunk &chunk, VmOpCounts &c)
{
        ...
        for (const Instr &in : chunk.code) {
            ...
            case OpCode::CheckFuncV:       c.checkfunc++; break;
            case OpCode::MapFilterV:       c.mapfilter++; break;
            case OpCode::LoadConstV:       c.loadconstv++; break;
            case OpCode::MoveV:            c.movev++; break;
            case OpCode::BinOpV:           c.binopv++; break;
            case OpCode::CompoundV:        c.compoundv++; break;
...
static bool vm_codegen_shapes()
{
    ...
    const bool fallback_ok =
        b.juic == 1 && b.intbin >= 1 && b.makeclosure >= 1;

    /* 2b) `show()` - the LAST node-ABI builtin - FOLDS at compile time
     * (fold_show_calls: a named function renders via render_func_code, a
     * non-identifier expression via render_construct_code, both -> a
     * LiteralStr), so it NEVER reaches codegen (there is no fallback op to
     * reach). Here the folded string makes the whole while-condition const
     * -> the loop DCEs / compiles with zero loop ops. */
    VmOpCounts b2;
    if (!codegen_counts({
            "func g2() => 1;",
            "var i = 0;",
        ...
        return false;
    /* codegen SUCCEEDING is the pin: an unfolded show() has no ABI and
     * would abort compilation (throw_not_lowered); the fold left only a
     * LiteralStr, so the loop lowers like any const-condition while. */
    const bool fallback_net_ok = b2.halt == 1;
    ...
    /* 10) an array-element WRITE `a[i] = expr` compiles to a native StoreElemInt
     * (int) / StoreElemFloat (float), with the read still a LoadElem (Phase 5). */
    VmOpCounts wi;
    if (!codegen_counts({
            "var a = array(5, 0);",
            "for (var i = 0; i < 5; i++) a[i] = i*2;",
        }, wi))
        return false;
    const bool write_int_ok =
        wi.storei == 1 && wi.flstep == 1 && wi.loadei == 0;

    VmOpCounts wf;
    if (!codegen_counts({
            "var a = array(3, 0.0);",
            "for (var i = 0; i < 3; i++) a[i] = i*1.5;",
        }, wf))
        return false;
     ...
     * fallback op even exists - zero-AST by construction). */
    VmOpCounts nl;
    if (!codegen_counts({
     ...
     * and drops the result (map lowers via CheckFuncV + MapFilterV - its
     * validate-arg0-before-arg1 order preserved), so the counted `for`
     * (ForLoopStep) has NO fallback op - there is none to have. */
    VmOpCounts ab;
    if (!codegen_counts({
            "var s = 0; var a = [3, 1, 2];",
    ...
    const bool array_build_ok =
        ab.flstep == 1 && ab.intbin >= 1 && ab.mapfilter == 1
        && ab.checkfunc == 1;
    ...
    /* 20) boxed COMPOUND-assign: `s += "x"` -> a CompoundV read-modify-write
     * (num_bin_op in place), not a fallback EvalStmt. */
    VmOpCounts bxc;
    if (!codegen_counts({
            "var s = \"\";",
            "for (var i = 0; i < 5; i++) { s += \"x\"; }",
        }, bxc))
        return false;
    const bool boxed_compound_ok =
        bxc.compoundv == 1 && bxc.loadconstv >= 1 && bxc.flstep == 1
       ;
     ...
     * fused ForLoopStep - no EvalStmt for the loop itself (the `s = s + x` body
     * is a native IntBin). */
    VmOpCounts fe;
    if (!codegen_counts({
            "var a = [1, 2, 3];",
    ...
    if (!codegen_counts({
        ...
        return false;
    const bool bool_cond_ok = bvc.jutv >= 1;
    ...
    if (!codegen_counts({
        ...
        return false;
    const bool var_init_ok = vif.movev >= 1;
    ...
    if (!codegen_counts({
        ...
        return false;
    const bool block_ok = blk.intbin >= 2;
    ...
    if (!codegen_counts({
        ...
        return false;
    const bool dict_member_ok = dms.dictstore >= 1;
    ...
    if (!codegen_counts({
        ...
        return false;
    const bool nested_store_ok = n2.storeelem2 >= 1;
    ...
    if (!codegen_counts({
        ...
        return false;
    const bool global_store_ok = gs.storei >= 1;
    ...
    const bool chain_finally_native_ok =
        cf.pushhandler == 2 && cf.endfinally == 2;
    ...
    if (!codegen_counts({
        ...
        return false;
    const bool unary_val_ok = unv.unaryv == 1;
    ...
    if (!codegen_func_counts({
        ...
        return false;
    const bool coalesce_ok = co.jinn == 1;
    ...
    if (!codegen_counts({
        ...
        return false;
    const bool chained_cmp_ok = ch.cmpv == 2;
    ...
    if (!codegen_counts({
        ...
        return false;
    const bool assign_expr_ok = ax.binopv == 2;
    ...
    if (!codegen_counts({
        ...
        return false;
    const bool typed_unpack_ok = mu.munpack == 1;
    ...
    if (!codegen_func_counts({
        ...
        return false;
    const bool expr_body_chunk_ok = eb.retv == 1;
    ...
    if (!codegen_counts({
        ...
        return false;
    const bool incdec_chain_ok = idc.idchain == 1;
...
ast_node_pool_minimal()
{
    ...
    /* ZERO-AST by construction: the node_table/ast_nodes members are DELETED
     * from Chunk, so the only checkable residue is a live per-Instr node_idx -
