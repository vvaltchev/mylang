...
static const std::vector<test> tests =
{
    ...
    { "catchd: try/catch inside a CATCH body",
      { "struct A { int v; } struct B { int v; }",
        ...
        "}",
        "assert(trc == [1, 2, 3]);" } },
    /* ("try/catch inside a FINALLY body" and its siblings - the nested
     * pend/exc CLOBBER class the battery exposed - land with the pend-stack
     * fix, task #78 step 2: the VM diverges from the tree-walker on them
     * today.) */
