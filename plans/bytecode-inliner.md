...
**DEFAULT ON since 2026-08-02** (`-nbi` / `MYLANG_BCINLINE=0` is the kill
switch). Measured -37.9% Ir on 10_recursion_deep with the JIT on (882.7M ->
547.8M; wall 0.14s -> 0.08s), and EXACTLY 0.00% on the untouched call-heavy
benches - a declining pass costs a compile-time scan and nothing at runtime.
The reach is still narrow (2 eligible sites across bench/ + samples) and the
dominant blocker is the runtime callee, 10 of 19 sites, which needs an inline
cache - see the reach note below.

It runs correctly with the JIT off. **All three defects below are now
...
`rand()`). The flip landed on 2026-08-02.
