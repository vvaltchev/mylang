...
**Update (2026-07-04): ~3.8x → ~3.7x, then FIXED to ~3.9x** — the builtin
VALUE-ABI migration slipped the `-vm` geomean, but the root cause was NOT the
adapter (an early wrong guess). Measured with cachegrind against the
pre-migration baseline: it was an **INLINING regression**. Adding the
`CallBuiltinV` handler grew `vm_run_chunk` past the compiler's inline budget, so
it stopped inlining the tiny hot per-op operand helpers (`read_int_operand` /
`write_int_slot` / `read_float_operand` / `write_float_slot`) — splitting each
into a real function and adding a call+return to EVERY int/float op, builtins or
not (so even `01_while_loop`, which has none, regressed +11%). A secondary cost:
`CallBuiltinV` (and `builtin_v_adapter`) built a `std::vector` arg buffer on
EVERY call, even for the common ≤8-arg case. **Fix (commit after the migration
batches):** `ML_ALWAYS_INLINE` (defs.h) pins the four operand helpers into the
loop regardless of its size (robust as more opcodes land), and the >8-arg heap
buffer moved to an `ML_NOINLINE` cold helper so the hot path carries no vector
and `vm_run_chunk` stays small. Result (cachegrind I-refs, `-vm`, vs baseline):
`01_while_loop` −11%, `04_float_arith` −14%, `38_min_max` −11%,
`31_str_split_join` −3%; `40_math_builtins` (5 builtin calls/iter, the worst
case) still +9%. Broad `-vm` geomean **3.7x → 3.9x**, now ABOVE the 3.8x
pre-migration baseline — the migration is net-positive. **Lesson:** adding code
to a hot dispatch function can de-inline its leaf helpers; force-inline the
hottest leaves and keep cold setup (vectors, error paths) out of the function.
The `40_math_builtins` +9% residual (the `CallBuiltinV` arg-copy + result-store,
